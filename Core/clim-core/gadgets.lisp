...
(defmethod* (setf scroll-bar-values)
    (min-value max-value thumb-size value (pane scroll-bar-pane))
  (setf (slot-value pane 'min-value) min-value
	(slot-value pane 'max-value) max-value
	(slot-value pane 'thumb-size) thumb-size
	(slot-value pane 'value) value)
  (with-slots (all-new-p) pane (setf all-new-p t) (scroll-bar/compute-display pane)))
